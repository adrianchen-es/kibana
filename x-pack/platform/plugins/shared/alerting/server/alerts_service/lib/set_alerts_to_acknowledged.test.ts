...
import { setAlertsToAcknowledged } from './set_alerts_to_acknowledged';
...
describe('setAlertsToAcknowledged()', () => {
  beforeEach(() => {
    jest.useFakeTimers();
  ...
  test('should call updateByQuery on provided ruleIds', async () => {
    ...
    expect(clusterClient.updateByQuery.mock.lastCall).toMatchInlineSnapshot(`
      ...
      if (!ctx._source.containsKey('kibana.alert.status') || ctx._source['kibana.alert.status'].empty) {
        ctx._source.kibana.alert.status = 'acknowledged';
  ...
  test('should retry updateByQuery on failure', async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      'Attempt 1: Failed to acknowledge 2 of 10; indices test-index, ruleIds test-rule'
    );
  });
  ...
  describe('ensureAuthorized', () => {
    test('should fail on siem consumer', async () => {
      ...
      await expect(
        ...
        `"Acknowledging Security alerts is not permitted"`
      );
    });
  ...
  test('should untrack by query', async () => {
    ...
    const result = await setAlertsToAcknowledged({
      isUsingQuery: true,
      query: [
        {
          bool: {
            ...
            must: [
              {
                term: {
                  'kibana.alert.status': {
                    value: 'acknowledged', // This has to be acknowledged
                  },
                },
              },
            ],
  ...
  test('should acknowledge by query', async () => {
    getAllAuthorizedRuleTypesFindOperationMock.mockResolvedValue(
      new Map([
        [
          'test-rule-type',
